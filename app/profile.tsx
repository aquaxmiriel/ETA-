...
export default function ProfileScreen() {
	...
	useEffect(() => {
		const loadUserData = async () => {
			...
			console.log("USERRRR", name, email, userId, "USERRRR", user);
					...
					onPress: async () => {
						try {
							...
							const confirmRes = await fetch(
								"https://expenses-tracker-8k6o.onrender.com/api/bank/confirm-transaction",
								{
									method: "POST",
									headers: {
										"Content-Type": "application/json",
	...
	const handleExportPDF = async () => {
		...
		try {
			...
			const downloadResumable = FileSystem.createDownloadResumable(uri, destinationPath, {
				headers: {
	...
	return (
		<View style={styles.container}>
			...
			<TouchableOpacity
				style={[styles.addButton, { left: width / 2 - 35 }]}
